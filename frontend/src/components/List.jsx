...
function HeatmapLayer({ reports }) {
  ...
  useEffect(() => {
    ...
    const heatData = reports.filter(r => r.lat && r.lon).map(r => [r.lat, r.lon, 1]);
    const heat = L.heatLayer(heatData, { radius: 25, blur: 45, maxZoom: 8, max: 0.2 }).addTo(map);
...
function List({ reports, onSelectReport }) {
  ...
  return (
    <div>
      <div style={{ marginBottom: "1rem", background: "#fff", padding: "1rem", borderRadius: "8px", border: "1px solid #ddd" }}>
        ...
        <label style={{ display: "block" }}>
          Visa rapporter från senaste <strong>{timeFilter}</strong> dagarna
        </label>
        <input 
          type="range" 
          min="1" 
      ...
      <MapContainer center={[57.7089, 11.9746]} zoom={12} style={{ height: "400px", width: "100%", borderRadius: "12px", marginBottom: "2rem", border: "1px solid #ccc" }}>
        <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" attribution="© OpenStreetMap contributors" />
        
        {!heatmapOn && filteredReports.map((r) => r.lat && r.lon && (
